...
type ListLifterLevelsRow struct {
	UserID             int32 `json:"user_id"`
	QualifyingSessions int32 `json:"qualifying_sessions"`
}
...
func (q *Queries) ListLifterLevels(ctx context.Context) ([]ListLifterLevelsRow, error) {
	rows, err := q.db.Query(ctx, listLifterLevels)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListLifterLevelsRow
	for rows.Next() {
		var i ListLifterLevelsRow
		if err := rows.Scan(&i.UserID, &i.QualifyingSessions); err != nil {
			return nil, err
		}
