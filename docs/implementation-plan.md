...
| Levels | **Derived on read, never stored — the opposite call to Achievements, and for a reason rather than a taste.** A lifter's level comes from counting their qualifying sessions at request time (`db/queries/levels.sql`), with the curve in `internal/levels`. A crown is stored because deriving one costs the most expensive query in the app; a level is a grouped count over an indexed column, so there is no sweeper, no reconciler, and no window in which a written total disagrees with the sessions that explain it. That buys three things worth naming: a session **edited** after it is over cannot leave the total behind — and sessions are edited after they are over, on purpose; a session **deleted** takes its experience with it; and the award is **idempotent for free**, which matters because finishing a session is queued offline and replayed, so a write-time award would have to defend against paying twice for one workout. Generated lifters get levels without `activity.go` knowing the feature exists. **A session is worth a flat 100 XP** — not scaled by volume or sets, because the badge answers "has this person been showing up" and how strong somebody is is what the leaderboard and the crowns already say. The consequence is accepted rather than defended: a one-set session earns what a twenty-set session earns. **A session qualifies when it is over AND contains real work** — the `is_over` rule copied from `sessions.sql` (so `levels.sql` is one of the copies that file says to keep in sync), plus at least one set with `actual_reps > 0`. The work filter is what the ageing half needs: without it, opening a session and abandoning it pays out twelve hours later. It reads `actual_reps` and **not `completed`**, which means "hit its target" — qualifying on that would stop paying anybody who missed a rep. **The curve escalates**: level `L` costs `(L-1) x 100`, cumulative `50*L*(L-1)`, so level 12 is 66 sessions and level 30 is 435. **Level is deliberately NOT a field on `Lifter`**, for the reason Houses and Achievements both declined it: six queries hydrate a lifter for the wire and `users.sql` requires a column added to one roster query be added to the other. It is a site-wide `GET /levels` fetched once and consulted per name, like `/houses` and `/achievements` — and unlike those, **every account is listed**, including one that has never trained, so that "untrained" and "no such lifter" are not the same answer and the client never has to hold a copy of the starting level. **A level-up is announced, and a finish is pushed install-wide.** Finishing a session publishes a `level` frame, which is the only UNADDRESSED frame on the socket: a level is drawn beside a name everywhere one appears, so the set of clients holding a stale badge is every client, and the frame says nothing about whose level moved — which is what makes broadcasting it safe. It is published only when the call is what made the session count, so a repeat finish from the offline queue and a finish of an already-aged-out session both say nothing. The moment itself is client-side and has to be: nothing on the server knows a level changed, because a level is derived rather than written, so the difference exists only between two readings and `levelWatch.ts` is what holds both. **Beside a name it is the number alone; the progress into the current level is written out in exactly one place** — the Experience section of a lifter's profile (`LevelCard`, reused by the header's hover card). Drawn for whoever is being read about rather than only for the reader, on the rule the rest of that page follows: the prose changes person, the figures do not. Nothing is guarded by doing otherwise, since XP is a qualifying-session count times a hundred and that count is a tile on the same page. **Milestone levels ARE achievements**, which is the one place this install deliberately holds two answers about one thing. Levels 5, 10, 20 and 30 are seeded as a second `AchievementKind` (`level`) with a `level_threshold` column, and reaching one opens a reign that **never closes** — see migration 0035. The line that makes this compatible with the rule forbidding achievements for derived facts is **standing versus crossing**: a personal record is a standing, and a stored copy beside the sessions can contradict them, which is why records are still not achievements; "passed Level 10 on the 14th of March" is an event with a date that no later edit makes untrue. The accepted cost is stated rather than hidden — a lifter who deletes sessions sits at Level 9 holding the Level 10 rung, and `/levels` and the profile disagree ON PURPOSE. Awarded twice over for one reason: in the `finishSession` transaction, so the moment is immediate, and on the hourly sweeper, which is the only thing that can see a rung crossed by a session that aged out with nobody finishing it. Both are idempotent through `OpenAchievementReign`'s unique partial index, so neither pays twice. **0035 backfills the reigns**, unlike 0032, and that is what makes the first pass silent structurally rather than by a flag somebody has to remember: history here IS knowable from the sessions, so nobody is told about a rung they passed months ago. `CloseAchievementReignsExcept` is keyed on the slug alone and has no notion of a permanent award — the rail against pointing it at a level is that `refreshLevelAwards` never calls it. Two UI filters are load-bearing rather than tidy: `crownsFor` and `noteCrowns` both had to learn `kind`, or every lifter past Level 5 would wear a bogus Crown icon beside their name on every surface and be congratulated for "taking a crown" on the first poll after deploy. |
