...
> lives. What it would buy on top of Fix B: `go mod tidy` as a local gate, and an
> end to the baked-store staleness coupling described under "The catch".
>
> **`sqlc` is no longer an example of what Fix A would buy** (2026-09-13). The Go
> module proxy *is* reachable from the sandbox, so `go install` works today —
> `internal/store` does not have to be regenerated by hand. What stops a plain
> `go install` is only that `/opt/go/pkg/mod` and `/usr/local/gobin` are read-only,
> which redirecting `GOMODCACHE`/`GOPATH`/`GOBIN` gets around. The command is in
> [`docs/development.md`](./development.md#regenerating-the-data-layer-sqlc).
